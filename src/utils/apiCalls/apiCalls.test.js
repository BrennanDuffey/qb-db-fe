...
describe('apiCalls', () => {
  ...
  describe('fetchWithOptions', () => {
    it('should be called with the correct pararms', () => {
      const expected = 'http://localhost:3000/api/v1/tossups/19&25/15';
      fetchWithOptions(mockCount, mockSelectedCategories);
      expect(window.fetch).toHaveBeenCalledWith(expected)
    });

    it('should return correct response if ok', async () => {
      const result = await fetchWithOptions(mockCount, mockSelectedCategories);
      expect(result.tossups).toEqual(mockTossups);
    });
